defmodule Bluette.Accounts.Wallet.SolanaTest do
  use ExUnit.Case, async: true

  alias Bluette.Accounts.Wallet.Solana

  test "verifies an ed25519 signature from the matching address" do
    {address, signature, message} = sign("Sign in to Bluette\nNonce: abc123")

    assert Solana.verify(message, signature, address)
  end

  test "rejects a signature from a different key" do
    {address, _signature, message} = sign("Sign in to Bluette\nNonce: abc123")
